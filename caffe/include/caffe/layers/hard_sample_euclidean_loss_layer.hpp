#ifndef CAFFE_HARDSAMPLEEUCLIDEAN_LOSS_LAYER_HPP_
#define CAFFE_HARDSAMPLEEUCLIDEAN_LOSS_LAYER_HPP_
 ...
 * input to a HardSampleEuclideanLossLayer exactly formulates a linear least squares
...
class HardSampleEuclideanLossLayer : public LossLayer<Dtype> {
 public:
  explicit HardSampleEuclideanLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "Hard Sample EuclideanLoss"; }
  /**
   * Unlike most loss layers, in the HardSampleEuclideanLossLayer we can backpropagate
 ...
 protected:
  /// @copydoc HardSampleEuclideanLossLayer
